...
} from "medusa-plugin-bundles-client"
...
export async function getBundlesList({
  ...
  const { bundles, count, nextPage } = await medusaRequest("GET", "/bundles", {
    query: {
      limit,
      offset: pageParam,
      ...queryParams,
 ...
 * @returns 'nextPage' (number) - The offset of the next page of products
 */
export async function getBundleProductsList({
  id,
  pageParam = 0,
  queryParams = {},
}: {
...
/**
 * Fetches a bundle by handle, using the Medusa API.
 * @param handle  (string) - The handle of the bundle to retrieve
 * @returns bundles (array) - An array of bundles (should only be one)
 * @returns response (object) - An object containing the products and the number of products in the collection
 * @returns nextPage (number) - The offset of the next page of products
 */
...
/**
 * Fetches a list of products in a bundle, using the Medusa API.
 * @param pageParam (number) - The offset of the products to retrieve
 * @param handle (string) - The handle of the collection to retrieve
 * @param cartId (string) - The ID of the cart
 * @returns response (object) - An object containing the products and the number of products in the bundle
 * @returns nextPage (number) - The offset of the next page of products
 */
export async function getProductsByBundleHandle({
  pageParam = 0,
  handle,
  cartId,
}: {
  ...
  const { id } = await getBundleByHandle(handle).then((res) => res.bundles[0])

  const { response, nextPage } = await getBundleProductsList({
    id,
    pageParam,
