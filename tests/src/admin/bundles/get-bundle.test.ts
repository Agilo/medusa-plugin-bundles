...
function testAndSanitizeData(data: any) {
  ...
  recursiveStripProps(data, [
    ...
    "data.bundle.products.created_at",
    "data.bundle.products.updated_at",
...
describe("/admin/bundles/get-bundle", () => {
  ...
  it("should return a bundle", async () => {
    ...
    expect({ data, status: response.status }).toMatchFileSnapshot(
      `../../fixtures/admin/bundles/get-bundle/get-bundle-${id}.json`
    );
  });
  ...
  it("should return a bundle even if it is not published", async () => {
